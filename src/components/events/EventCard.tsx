...
const EventCard: React.FC<EventCardProps> = ({ title, description, date, progress, accentColor, isActive }) => {
  return (
    <div
      className="relative rounded-2xl transition-all duration-300 w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl flex items-center justify-center p-0 m-0"
      style={{
        boxShadow: isActive
          ? `0 1px 16px 4px ${accentColor}`
          : `0 1px 10px 1px ${accentColor}`
      }}
    >
